...
def count_by_fleet_refuel(table_name, fleets=None):
    base_sql = f"""
        SELECT f.fleet_name, COUNT(*) AS n_points
        FROM {table_name} AS t
        JOIN charger as c
...
def count_by_fleet_maintenance(table_name="maintenance", fleets=None):
    base_sql = f"""
